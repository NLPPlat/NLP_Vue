<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.arrangeName" placeholder="部署名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      ...
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleArrangeCreate">
        生成部署
      </el-button>
    </div>
    ...
    >
      ...
      <el-table-column label="部署名称" width="180px" align="center">
        <template slot-scope="{row}">
          <el-popover
            placement="right"
            width="400"
          ...
          >
            <el-form>
              <el-form-item label="部署名称">
                <el-input v-model="row.arrangeName" />
              </el-form-item>
              <el-form-item label="调用接口">
                <el-input v-model="row.url" :disabled="true" />
              </el-form-item>
            ...
            <span slot="reference" class="link-type">{{ row.arrangeName }}</span>
          </el-popover>
        </template>
      </el-table-column>
      ...
      <el-table-column label="适用任务类型" column-key="taskType" :filters="taskTypeFilter" width="160px" align="center">
        <template slot-scope="{row}">
          <el-tag>{{ row.taskType | typeFilter }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="部署状态" column-key="arrangeStatus" :filters="statusFilter" class-name="status-col" width="100px">
        <template slot-scope="{row}">
          <el-tag :type="row.arrangeStatus | statusFilter">
            {{ row.arrangeStatus }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" min-width="300px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <div v-if="permissionCheck(row.username)">
            ...
            <el-button v-if="row.arrangeStatus==='关闭'" type="success" size="mini" @click="handleStart(row)">
              开始运行
            </el-button>
            <el-button v-else type="danger" size="mini" @click="handleClose(row)">
              停止运行
            </el-button>
    ...
    <arrange-create ref="ArrangeCreate" />
  </div>
</template>

<script>
import { datafileInfoUpdate } from '@/api/common/datafile'
import { arrangeFetch } from '@/api/common/arrange'
import { writePermission } from '@/utils/permission'

import ArrangeCreate from './arrange-create'
import Pagination from '@/components/Pagination'

export default {
  name: 'ArrangeTable',
  components: { Pagination, ArrangeCreate },
  filters: {
    statusFilter(status) {
      const statusMap = {
        '运行中': 'primary',
  ...
  methods: {
    ...
    getList() {
      ...
      arrangeFetch(this.listQuery).then(response => {
    ...
    // 资源管理系列函数
    handleArrangeCreate() {
      this.$refs.ArrangeCreate.showDialog()
    },
    handleStart(row) {
      datafileInfoUpdate({ 'datafileid': row._id, 'datafileType': '部署', 'infos': { 'arrangeStatus': '运行中' }}).then(response => {
        this.$message.success('部署开始运行!')
        row.arrangeStatus = '运行中'
      })
    },
    handleClose(row) {
      datafileInfoUpdate({ 'datafileid': row._id, 'datafileType': '部署', 'infos': { 'arrangeStatus': '关闭' }}).then(response => {
        this.$message.success('部署关闭成功!')
        row.arrangeStatus = '关闭'
      })
    },
    handleManage(row) {
      this.$router.push('/expand/arrange/arrange-manage/' + row._id)
    },
    handleDelete(row) {
    },
    handleInfoVerity(row) {
      datafileInfoUpdate({ 'datafileid': row._id, 'datafileType': '部署', 'infos': { 'arrangeName': row.arrangeName, 'desc': row.desc }}).then(response => {
        document.body.click()
        this.$message.success('部署信息修改成功！')
      })
    },
    handlePublicityChange(row) {
      datafileInfoUpdate({ 'datafileid': row._id, 'datafileType': '部署', 'infos': { 'publicity': row.publicity }}).then(response => {
